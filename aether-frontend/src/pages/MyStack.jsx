...
export default function MyStack() {
        ...
        const fetchStack = async () => {
            ...
            try {
                ...
                const validTools = results.filter(Boolean).map((data, idx) => ({
                    id: String(idx),
                    name: data.name,
                    category: data.analysis?.job_to_be_done?.[0] || 'General',
    ...
    return (
        <div className="flex flex-col w-full min-h-[85vh] animate-in fade-in duration-700 ltr" dir="ltr">
            ...
            <div className="w-full mb-10 pb-8 border-b border-white/10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-white/60 text-sm">Manage your saved tools and active system configurations.</p>
                </div>
            ...
            <div className="w-full flex justify-between items-center mb-6">
                <div className="relative w-full max-w-sm">
                    ...
                    <input
                        ...
                        className="w-full bg-white/5 backdrop-blur-md border border-white/20 rounded-xl pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-neutral-900/10 focus:border-white/20 transition-all shadow-sm"
                    />
                </div>
            ...
            {filteredTools.length === 0 && (
                <div className="flex flex-col items-center justify-center p-12 bg-white/5 backdrop-blur-md border border-white/20 border-dashed rounded-3xl mt-4">
                    ...
                    <h3 className="text-lg font-medium text-white mb-1">No tools found in stack</h3>
                    <p className="text-sm text-white/60 text-center max-w-sm">
                        You haven't saved any tools yet or your search returned no results. Build your perfect AI stack from the vault.
                    </p>
                </div>
            )}
            ...
            {filteredTools.length > 0 && viewMode === 'grid' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 animate-in slide-in-from-bottom-4 duration-500">
                    {filteredTools.map(tool => (
                        <div key={tool.id} className="group flex flex-col p-5 bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl shadow-sm hover:border-white/20 transition-all hover:shadow-md cursor-pointer relative overflow-hidden text-left">
                            ...
                            <div className="flex items-center gap-2 mt-auto pt-6 border-t border-white/10 w-full justify-between">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs text-white/60">{tool.isActive ? 'Active in Team' : 'Saved for Future'}</span>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {filteredTools.length > 0 && viewMode === 'list' && (
                <div className="flex flex-col w-full bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden shadow-sm animate-in slide-in-from-bottom-4 duration-500 text-left">
                    <div className="flex w-full bg-white/5 backdrop-blur-md border-b border-white/20 px-6 py-3 text-xs font-semibold text-white/60 uppercase tracking-wider">
                        ...
                        <div className="w-32 hidden md:block text-left">Category</div>
                        <div className="w-32 text-center">Trust Score</div>
                    ...
                    {filteredTools.map((tool, idx) => (
                        <div key={tool.id} className={`flex w-full px-6 py-4 items-center group cursor-pointer hover:bg-white/5 backdrop-blur-md transition-colors ${idx !== filteredTools.length - 1 ? 'border-b border-white/10' : ''}`}>
                            ...
                            <div className="w-32 hidden md:block text-sm text-white/60 text-left">{tool.category}</div>
