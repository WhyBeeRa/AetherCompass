...
export default function Architect() {
    const [workflowName, setWorkflowName] = useState('פרויקט חדש');
    const [steps, setSteps] = useState([
        { id: 1, tool: 'Whisper Large V3', action: 'המרת פודקאסט לטקסט', config: 'Precision Mode' },
        { id: 2, tool: 'GPT-4o', action: 'סיכום לנקודות מפתח', config: 'Concise' }
    ]);
            ...
            <main className="max-w-7xl mx-auto px-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-16">
                    <div>
                        ...
                        <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-2 text-center md:text-right">
                            AI <span className="text-emerald-500">Architect</span>
                        </h1>
                        <p className="text-white/50 font-medium text-center md:text-right">
                            אל תאסוף כלים, תבנה מכונות. אנחנו נותנים לך את תוכנית העבודה המלאה.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-4">
                        <button className="flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 rounded-xl font-bold text-sm hover:bg-white/10 transition-all">
                            <Save className="w-4 h-4" /> שמור Blueprint
                        </button>
                        <button className="flex items-center gap-2 px-8 py-3 bg-emerald-500 text-black rounded-xl font-black text-sm hover:bg-emerald-400 shadow-lg shadow-emerald-500/20 transition-all">
                            <Play className="w-4 h-4 fill-current" /> הרץ Workflow
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                    ...
                    <div className="md:col-span-3 space-y-6">
                        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
                            <h3 className="text-xs font-black text-white/30 uppercase tracking-[0.2em] mb-6">ספריית רכיבים</h3>
                            <div className="space-y-3">
                                {['המרת אודיו', 'עיבוד שפה', 'יצירת תמונה', 'עריכת וידאו', 'אוטומציה (Zapier)'].map((cat, i) => (
                        ...
                        <div className="bg-gradient-to-br from-emerald-500/20 to-transparent border border-emerald-500/20 rounded-2xl p-6">
                            ...
                            <p className="text-[10px] text-white/50 font-medium leading-relaxed mb-4">
                                המערכת מזהה כי ניתן לחסוך $5.20 חודשיים בשרשרת הזו על ידי החלפת Whisper ב-Groq API.
                            </p>
                    ...
                    <div className="md:col-span-9 relative">
                        ...
                        <div className="space-y-8 relative">
                            {/* Connection Line */}
                            <div className="absolute top-10 right-1/2 w-0.5 h-[calc(100%-80px)] bg-gradient-to-b from-emerald-500/50 to-transparent pointer-events-none hidden md:block"></div>

                            {steps.map((step, idx) => (
                                <div key={step.id} className="relative z-10">
                                    <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[2rem] p-8 hover:bg-white/[0.05] transition-all group border-r-4 border-r-emerald-500/50">
                            ...
                            >
                                <Plus className="w-6 h-6" /> הוסף שלב לשרשרת
                            </button>
                        </div>
                    </div>
